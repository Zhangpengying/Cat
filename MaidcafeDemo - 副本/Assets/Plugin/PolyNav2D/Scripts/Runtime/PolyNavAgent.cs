...
public class PolyNavAgent : MonoBehaviour{
	...
	///The rate at which it will accelerate(加速时的加速度)
	public float accelerationRate    = 0;
	///The rate at which it will slow down（减速时的加速度）
    ...
    //减速和避让的前视距离。设置为0可以消除减速但也可以避免减速，以及提高性能。
    public float lookAheadDistance    = 0;
    ///Should the agent repath? Disable for performance.代理是否应该做出回应?禁用的性能。
	...
	public bool SetDestination(Vector2 goal, Action<bool> callback){
        ...
        //调整朝向
        transform.localScale = goal.x - _transform.localPosition.x < 0 ? new Vector3(-1, 1, 1) : new Vector3(1, 1, 1);
